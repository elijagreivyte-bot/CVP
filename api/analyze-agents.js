...
Grąžink TIK JSON:
{
  ...
  "trukstamiDuomenys": ["ko trūksta, kad analizė būtų pilna"]
    ...
    // ── VIDINIS WATCHDOG'AS ──
    // Vercel Hobby planas kietai nutraukia funkciją po 60s IŠORIŠKAI — tokiu atveju
    // grąžinama ne-JSON klaidos žinutė, kurią frontend'as rodo kaip "neįskaitomas (500)".
    // Šis watchdog'as suveikia PRIEŠ tą kietą ribą ir grąžina TEISINGĄ, sąžiningą JSON
    // atsakymą — nesvarbu, kokio dydžio dokumentas buvo įkeltas.
    const HARD_WATCHDOG_MS = 54000; // 6s marža iki Vercel Hobby 60s kietos ribos
    let watchdogTimer;
    const watchdogPromise = new Promise((resolve) => {
      watchdogTimer = setTimeout(() => resolve({ __timeout: true }), HARD_WATCHDOG_MS);
    });
    const raced = await Promise.race([runPipeline(), watchdogPromise]);
    clearTimeout(watchdogTimer);

    if (raced && raced.__timeout) {
      // Grąžinam rezervuotą nemokamą analizę — vartotojas negavo pilno rezultato (best-effort)
      ...
      return res.status(200).json({
        result: {
          pavadinimas: documentName || 'Analizė',
          _partial: true,
          _timedOut: true,
          score: 50,
          scoreLabel: 'Nepavyko įvertinti laiku',
          sprendimas: 'CLARIFY',
          sprendimoPriezastis: 'Dokumentas per didelis arba sudėtingas pilnai išanalizuoti per vieną užklausą.',
          isViso: 'Dokumentas (arba dokumentų rinkinys) per didelis, kad AI spėtų jį pilnai išanalizuoti per vieną užklausą. Rekomenduojame: (1) įkelti dokumentus atskirai, o ne vieną ZIP archyvą — tai leidžia sistemai apdoroti kiekvieną failą nuosekliai, (2) pirmame etape analizuoti tik pirkimo sąlygas ir techninę specifikaciją, likusius priedus (sutarties projektą, priedus, žiniaraščius) aptarti vėliau per AI asistento pokalbį, jau turint pagrindinę analizę.',
          patikimumoIndikatorius: { procentas: 15, priezastys: ['Analizė nutraukta dėl laiko limito — dokumentas per didelis vienai užklausai apdoroti'] }
        }
      });
    }
...
// tas nustatymas laikomas viršesniu Hobby plane. Nuo šiol tai nebekritiška:
// vidinis watchdog'as (HARD_WATCHDOG_MS=54000, žr. aukščiau) visada grąžina
// teisingą JSON PRIEŠ Vercel spėja priverstinai nutraukti funkciją — net jei
// dokumentas per didelis, kad AI spėtų jį pilnai apdoroti per 60s.
